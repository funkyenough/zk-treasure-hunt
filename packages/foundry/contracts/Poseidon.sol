/// @dev referred to https://github.com/iden3/circomlibjs/blob/v0.0.8/src/poseidon_slow.js
...
/// @title Poseidon hash function implementation
...
contract Poseidon {
     ...
     */
    uint constant N_ROUNDS_F = 8;
    uint constant N_ROUNDS_P = 57;
    ...
    function pow5(uint n) internal pure returns (uint) {
        uint pow2 = mulmod(n, n, F);
        uint pow4 = mulmod(pow2, pow2, F);
    ...
    function hash(uint[2] memory inputs) public pure returns (uint) {
        ...
        uint[3] memory res = [0, inputs[0], inputs[1]];
